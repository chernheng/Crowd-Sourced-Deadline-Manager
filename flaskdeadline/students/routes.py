...
def feedback(module,cw):
    ...
    # providing feedback to coursework
...
def subscribed(module_id):
    ...
    if not session['samlUserdata']:
        ...
        if not user:
            flash('User does not exist!', 'danger')
            return redirect(url_for('main.landing'))
    # Add to module taken if does not already exist, remove it from the list if it does
...
def new_mod():
    ...
    if not session['samlUserdata']:
        ...
        if not user:
            ...
            return redirect(url_for('main.landing'))
    if form.validate_on_submit():
        # check if id or title already exist, cannot create new module if either exist
...
def edit_mod(module_title):
    ...
    original_ects = mod.ects
    check_id = check_title = False
    if form.validate_on_submit():
        # If the title, id and ects is the same, just redirect it home
        if original_title == form.title.data and original_id == form.id.data and original_ects == form.ects.data:
            return redirect(url_for('students.home'))
...
def intensity():
    ...
    return render_template("lin_opt.html",  form = form, display = start_dates_dict, dates = start_end_dates, avail_modules = avail, taking = user.module_taken, user=user, label=label,date=data, cwk= names)
...
def home():
    ...
    # Modules subscribed by user without deadline
    no_deadline_mod = []
    ...
    '''
    All deadlines subscribed by student in this form:
    ...
    # coursework start dates for module subscribed by students
    ...
    # Checking the average number of hours students spent for each coursework, average hours and how many people voted
    no_hours = {}
